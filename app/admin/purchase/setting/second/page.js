...
export default function Home() {
  ...
  async function getPurchase(id) {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL_8102}/fjbc_tutoring_api/purchase?id=${id}&state=3`, config);
  ...
  async function pass() {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL_8102}/fjbc_tutoring_api/purchase/step3/pass?id=${purchase.id}`, config);
    const res = await response.json();
    if (response.ok) {
      setInfo({
        show: true,
  ...
  return (
    <>
      <Alert
        ...
        setInfo={setInfo}
      />
      <div className="px-2 mx-auto p-2">
        ...
        <div className="bg-white p-4 rounded-md">
          <div className="grid grid-cols-6 gap-4">
            <div className="col-span-1 text-lg font-semibold">
              ...
              <div className="text-blue-800">狀態：待複審</div>
            </div>
            <div className="col-span-4 flex my-4 h-8 m-auto">
              ...
              <div className={`w-40 border-b-2 relative color-step`}>
                <div className="absolute lineStr font-semibold">待複審</div>
