...
export default function Home({ setInfo }) {
  ...
  async function getData() {
    ...
    let api1 = `${process.env.NEXT_PUBLIC_API_URL_8102}/fjbc_tutoring_api/tutoring/rollcall_teacher/list`;
    let api2 = `${process.env.NEXT_PUBLIC_API_URL_8102}/fjbc_tutoring_api/tutoring/rollcall_student/list`;
    if (searchDate != "") {
      api1 += `?date=${searchDate}`;
      api2 += `?date=${searchDate}`;
    }
  ...
  async function updateRollcall() {
    ...
    let api = "";
    if (createData.type) {
      api = `${process.env.NEXT_PUBLIC_API_URL_8102}/fjbc_tutoring_api/tutoring/rollcall_teacher/${createData.id}/update`;
    } else {
      api = `${process.env.NEXT_PUBLIC_API_URL_8102}/fjbc_tutoring_api/tutoring/rollcall_student/${createData.id}/update`;
    }
      ...
      >
        ...
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <div className="text-2xl text-blue-400 my-2 text-center">{createData.rollcall_status == 2 ? "補到班" : createData.rollcall_status == 3 ? "補離班" : ""}</div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setCreateData({
                        ...createData,
                        HH: e.target.value,
                        update_time: `${e.target.value}:${createData.mm}`
                      });
                    }}
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setCreateData({
                        ...createData,
                        mm: e.target.value,
                        update_time: `${createData.HH}:${e.target.value}`
                      });
                    }}
                    className="mt-2 block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm/6"
                  >
                ...
                <table className="min-w-full divide-y divide-gray-300 bg-white">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filteredTeacher.map((teacher) => (
                      ...
                      >
                        <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900">{teacher.course_name}</td>
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          {teacher.first_name}（{teacher.nick_name}）
                        </td>
                        ...
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium">
                          ...
                          >
                            補上課<span className="sr-only">, {teacher.name}</span>
                          </span>
                          <span
                            onClick={() => {
                              setCreateData({
                          ...
                          >
                            補下課<span className="sr-only">, {teacher.name}</span>
                          </span>
                        </td>
                      </tr>
                    ))}
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filteredStudent.map((student) => (
                      <tr
                        ...
                        className="hover:bg-blue-100"
                      >
                        <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900">{student.course_name}</td>
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          {student.first_name}（{student.nick_name}）
                        </td>
                        ...
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium">
                          <span
                            ...
                            className="text-green-600 hover:text-green-900 mx-2 cursor-pointer"
                          >
                            補點名<span className="sr-only">, {student.name}</span>
                          </span>
                          <span
                            onClick={() => {
                              setCreateData({
                            ...
                            className="text-red-600 hover:text-red-900 mx-2 cursor-pointer"
                          >
                            補下課<span className="sr-only">, {student.name}</span>
                          </span>
                        </td>
                      </tr>
                    ))}
