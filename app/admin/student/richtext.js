...
export default function Home({ createData, settingData, setSettingData }) {
  ...
  useEffect(() => {
    if (settingData) {
      setTextareaValue(settingData.health_description);
    }
  }, [settingData?.health_description]);

  useEffect(() => {
    if (createData) {
      createData.current.health_description = textareaValue;
    }

    if (settingData) {
      setSettingData({
        ...settingData,
        health_description: textareaValue
      });
    }
  }, [textareaValue]);
