...
export default function Home() {
  ...
  function getTableData() {
    ...
    rows.forEach((row) => {
      ...
      data.push({
        ...
        身份: cells[1].innerText,
        工時: cells[2].innerText,
        小計: cells[3].innerText
      });
    });
  ...
  function ExportToExcel() {
    ...
    const exportTableToExcel = () => {
      ...
      XLSX.writeFile(workbook, `${date.getFullYear()}_${date.getMonth() + 1}_${date.getDate()}_補習班時薪總表.xlsx`);
    };
  ...
  async function getItemGroup() {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL_8102}/fjbc_tutoring_api/wage/status/group`, config);
        ...
        <div className="px-4 mt-4">
          <div className="flex items-end justify-between">
            <div className="flex items-end">
              <h1 className="text-xl font-semibold text-gray-900">補習班時薪總表</h1>
