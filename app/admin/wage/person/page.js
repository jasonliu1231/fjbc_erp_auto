...
export default function Home() {
  ...
  const amount_time = list.reduce((val, item) => val + item.working_time, 0);
  const amount_money = list.reduce((val, item) => val + item.money, 0);
          ...
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <div className="grid grid-cols-4 gap-3 text-center">
                <div className="col-span-1 border-b-2">起始</div>
                <div className="col-span-1 border-b-2">{update.start_date}</div>
                <div className="col-span-1 border-b-2">結束</div>
                <div className="col-span-1 border-b-2">{update.end_date}</div>
                <div className="col-span-1 border-b-2">姓名</div>
                <div className="col-span-1 border-b-2">{update.name}</div>
                <div className="col-span-1 border-b-2">總工時 /分</div>
                <div className="col-span-1 border-b-2">{update.total_time}</div>
                <div className="col-span-1 border-b-2">應付小計</div>
                <div className="col-span-1 border-b-2">{update.total_money}</div>
                <div className="col-span-1 border-b-2">實付</div>
                <div className="col-span-1 border-b-2 text-red-400">{Number(update.total_money) + Number(update.bonus) + Number(update.plus)}</div>
                <div className="col-span-2">
                  <label className="text-gray-600">獎金</label>
                  <input
                    value={update.bonus}
                    onChange={(e) => {
                      setUpdate({ ...update, bonus: Number(e.target.value) });
                    }}
                    type="number"
                ...
                <div className="col-span-2">
                  <label className="text-gray-600">加給</label>
                  <input
                    value={update.plus}
                    onChange={(e) => {
                      setUpdate({ ...update, plus: Number(e.target.value) });
                    }}
                    type="number"
                    className="border-2 w-full px-2 py-1"
                  />
        ...
        <div className="px-4 mt-4">
          <div className="flex items-end justify-between">
            <div className="flex items-end">
              <h1 className="text-xl font-semibold text-gray-900">個人薪資明細表</h1>
            ...
            <div className="flex justify-end">
              {list.length > 0 && (
                <button
                  className="text-green-600 border-2 border-green-400 bg-green-100 px-2 py-1 rounded-md"
                  onClick={() => {
                    setUpdate({
                      ...update,
                      teacher_id: selectedPerson.teacher_id,
                      name: selectedPerson.name,
                      start_date: view.begin,
                      end_date: view.end,
                      total_time: amount_time,
                ...
                >
                  <thead>
                    <tr className="divide-x divide-gray-200 bg-green-100 sticky top-0">
                      ...
                      <th
                        scope="col"
                        className="py-2 pr-2 text-left text-sm font-semibold text-gray-900 text-right"
                      >
                        工時 /小時
                      </th>
                      <th
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {list.map((item, index) => {
                      return (
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap py-2 pl-2 text-sm text-gray-500">{item.check_in_time?.substr(0, 5)}</td>
                          <td className="whitespace-nowrap py-2 pl-2 text-sm text-gray-500">{item.check_out_time?.substr(0, 5)}</td>
                          ...
                          <td className="whitespace-nowrap py-2 pr-2 text-sm text-gray-500 text-right">{item.money}</td>
                        </tr>
                      );
                    })}
                    <tr className="bg-yellow-50">
                      ...
                      <td className="whitespace-nowrap text-sm font-medium text-gray-900 text-right">起始：</td>
                      <td className="whitespace-nowrap text-sm font-medium text-gray-900 text-left">{view.begin}</td>
                      <td className="whitespace-nowrap text-sm font-medium text-gray-900 text-right">結束：</td>
                      <td className="whitespace-nowrap text-sm font-medium text-gray-900 text-left">{view.end}</td>
                      <td className="whitespace-nowrap text-sm font-medium text-gray-900 text-right">總工時：</td>
                      <td className="whitespace-nowrap pr-2 text-sm font-medium text-gray-900 text-right">{amount_time}</td>
