...
export default function Example({ setInfo }) {
  ...
  async function getRollCall() {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL_8102}/fjbc_tutoring_api/tutoring/course_schedule_student/current/roll/list`, config);
    const res = await response.json();
    if (response.ok) {
      setStudentRollCall(res);
    } else {
      ...
      <div className="bg-white px-4 rounded-md h-80vh overflow-auto">
        <table className="min-w-full divide-y divide-gray-300">
          ...
          <tbody className="divide-y divide-gray-200 bg-white h-70vh overflow-auto">
            {studentRollCall.map((item, index) => {
              return (
                <tr
                  key={index}
